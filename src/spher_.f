...
C   any damages that may result from the use of the program. 
      ...
      INTEGER NMIE, NPL, L1, NRAD
      PARAMETER (NCOEF=10000, NRAD=5)
      PARAMETER (NMIE=NCOEF, NPL=2*NCOEF)
      REAL*8 AL1(NPL),AL2(NPL),AL3(NPL),AL4(NPL),BET1(NPL),           
     &     BET2(NPL), MRR,MRI,LAM, RAD(NRAD), WRAD(NRAD)
      REAL*8 AA, CEXT, CSCA, DDELT
...
C      WRITE(*,*) 'HELLO WORLD'
      ...
      CALL SPHER_(LAM, AA, MRR, MRI, DDELT, CEXT, CSCA, AL1, 
     &     AL2, AL3, AL4, BET1, BET2, L1)
      
      WRITE(*,*) 'compare with the the weighted sum call'
      WRITE(*,*) AL1(1:5)
      WRITE(*,*)

C      WRITE(*,*) LAM, AA, MRR, MRI, DDELT
      WRITE(*,*) 'CEXT=', CEXT, ',   CSCA=', CSCA
      WRITE(*,*) '=============================================='


C     define a vector of radii to sum over 
      RAD = [0.015D0, 3.D0, 5.D0, 7.D0, 9.D0]
      WRAD = [1.0D0, 0.0D0, 0.0D0, 0.0D0, 0.0D0]
      ...
      CALL SPHER_SUM(LAM, RAD, WRAD, NRAD, MRR, MRI, DDELT, 
     &     CEXT, CSCA, AL1, AL2, AL3, AL4, BET1, BET2, L1)
     
      WRITE(*,*) 'the example radius array is :'
      WRITE(*,*) RAD


      WRITE(*,*) 'the retured sum of gsf coef al1 starts with'
      WRITE(*,*) AL1(1:5) / CSCA
      WRITE(*,*) 'CEXT=', CEXT, ',   CSCA=', CSCA
C      WRITE(*,*) LAM, MRR, MRI, DDELT
      ...
      SUBROUTINE SPHER_SUM(LAM, RAD, WRAD, NRAD, MRR, MRI, DDELT,
     &     CEXT, CSCA, AL1, AL2, AL3, AL4, BET1, BET2, L1)
      
      IMPLICIT REAL*8 (A-H,O-Z)                                                 
c     INTEGER NMIE, NPL
      PARAMETER (NMIE=10000, NPL=2*NMIE)
      INTEGER NRAD, NRAD_, L1NEW
      REAL*8 AL1(NPL),AL2(NPL),AL3(NPL),AL4(NPL),BET1(NPL),           
     &     BET2(NPL), RAD(NRAD), WRAD(NRAD), MRR,MRI,LAM                                            
      REAL*8 AL1NEW(NPL),AL2NEW(NPL),AL3NEW(NPL),AL4NEW(NPL),
     &     BET1NEW(NPL), BET2NEW(NPL), CEXTNEW, CSCANEW
      
Cf2py intent(in) RAD, WRAD, LAM, AA, MRR, MRI, DDELT,                                 
Cf2py intent(out) CEXT, CSCA, AL1, AL2, AL3, AL4, BET1, BET2, L1      
...
C        Initialize the new arrays for this round
         L1NEW = 0
         CEXTNEW = 0.0D0
         CSCANEW = 0.0D0
         AL1NEW = 0.0D0
...
C        Compute the new values with a call to sphere_
         CALL SPHER_(LAM, RAD(NRAD_), MRR, MRI, DDELT,CEXTNEW, CSCANEW,
     &        AL1NEW, AL2NEW, AL3NEW, AL4NEW, 
     &        BET1NEW, BET2NEW, L1NEW)
         
C        Use the new values to update the sum
         CEXT = CEXT + CEXTNEW * WRAD(NRAD_)
         CSCA = CSCA + CSCANEW * WRAD(NRAD_)

         AL1 = AL1 + CSCANEW*AL1NEW * WRAD(NRAD_) 
         AL2 = AL2 + CSCANEW*AL2NEW * WRAD(NRAD_) 
         AL3 = AL3 + CSCANEW*AL3NEW * WRAD(NRAD_) 
         AL4 = AL4 + CSCANEW*AL4NEW * WRAD(NRAD_)

         BET1 = BET1 + CSCANEW*BET1NEW * WRAD(NRAD_)
         BET2 = BET2 + CSCANEW*BET2NEW * WRAD(NRAD_)
      ...
      RETURN 
      END SUBROUTINE SPHER_SUM





      SUBROUTINE SPHER_(LAM, AA, MRR, MRI, DDELT, CEXT, CSCA,
     &     AL1, AL2, AL3, AL4, BET1, BET2, L1)
...
Cf2py intent(in) LAM, AA, MRR, MRI, DDELT,                                 
Cf2py intent(out) CEXT, CSCA, AL1, AL2, AL3, AL4, BET1, BET2, L1
      BB=1D-1
      AA1=0.17D0
      AA2=3.44D0
      ...
      N=1
      NP=1
      R1=AA - 0.000000000001 D0
      R2=AA + 0.000000000001 D0
     ...
     *       RMEAN,AA1,BB1,AA2,BB2,DDELT)
      ...
      RETURN                                                         
      END SUBROUTINE SPHER_                                    

C***************************************************************
C     Define a subroutine for computing an average for a 
C     lognormal distribution. 
C***************************************************************

c$$$      subroutine spher_lognormal
c$$$
c$$$
c$$$      IMPLICIT REAL*8 (A-H,O-Z)                                                 
c$$$c      INTEGER NMIE, NPL
c$$$      PARAMETER (NMIE=10000, NPL=2*NMIE)
c$$$      REAL*8 AL1(NPL),AL2(NPL),AL3(NPL),AL4(NPL),BET1(NPL),           
c$$$     &     BET2(NPL),MRR,MRI,LAM                                            
c$$$
c$$$Cf2py intent(in) LAM, MRR, MRI, AA, BB, DDELT
c$$$Cf2py intent(OUT) CEXT, CSCA, AL1, AL2, AL3, AL4, BET1, BET2, L1
c$$$ 
c$$$      AA1=0.17D0
c$$$      AA2=3.44D0
...
c$$$      N=1
c$$$      NP=1
c$$$      R1=AA - 0.000000000001 D0
c$$$      R2=AA + 0.000000000001 D0
...
c$$$
c$$$      return 
c$$$      end subroutine spher_lognormal
...
C        180*(I-1)/(NPNA-1) (DEGREES), WHERE I NUMBERS THE ANGLES
      ...
      D6=DSQRT(6D0)*0.25D0
c      PRINT 1003
c 1003 FORMAT(' ',5X,'<',11X,'F11',6X,'     F33',6X,'     F12',
c     & 6X,'     F34')
      N=NPNA
      DN=1D0/DFLOAT(N-1)
      DA=DACOS(-1D0)*DN
...
c1001 FORMAT('MODIFIED GAMMA DISTRIBUTION, ALPHA=',F6.4,'  r_c=',               
c     &  F6.4,'  GAMMA=',F6.4)                                                   
      A2=AA/GAM                                                                 
      DB=1D0/BB
      DO 50 I=1,NNK                                                             
